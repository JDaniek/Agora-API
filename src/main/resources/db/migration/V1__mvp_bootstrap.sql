...
INSERT INTO notification_type_id (id, name)
VALUES (1, 'contact_request'),
       (2, 'new_chat_message'),
       (3, 'class_enrollment_request'),
       (4, 'class_enrollment_approved'),
       (5, 'class_enrollment_declined') ON CONFLICT (id) DO NOTHING;
...
(
    ...
    capacity_per_slot SMALLINT NOT NULL DEFAULT 1 CHECK
(
    capacity_per_slot >
    0
),
...
CREATE INDEX IF NOT EXISTS idx_classes_specialty_id ON classes (specialty_id);
-- <-- CORREGIDO

-- =========================================================
-- CLASS ENROLLMENTS (INSCRIPCIONES A CLASES)
-- PK corregida: (class_id, student_id)
-- =========================================================
...
(
  ...
  ON DELETE CASCADE,
    student_id BIGINT NOT NULL REFERENCES user_accounts
(
    id
)
  ON DELETE CASCADE,
...
(
    ...
    user_id BIGINT NOT NULL REFERENCES user_accounts
(
    id
)
  ON DELETE CASCADE,
...
(
    ...
    sender_id BIGINT NOT NULL REFERENCES user_accounts
(
    id
),
    body TEXT NOT NULL,
...
(
    ...
    created_at TIMESTAMPTZ NOT NULL DEFAULT now
(
) -- (Usamos TIMESTAMPTZ por consistencia)
    );
...
(
  ...
  ON DELETE CASCADE,
    notification_type_id INT NOT NULL REFERENCES notification_type_id
(
    id
),
    chat_id BIGINT NULL REFERENCES chats
(
    id
)
  ON DELETE CASCADE,
...
-- =========================================================
CREATE
OR REPLACE VIEW v_professors AS
...
COMMENT
ON TABLE specialty IS 'Catálogo de materias (ej. Matemáticas, Idiomas)';
COMMENT
ON TABLE user_specialties IS 'Tabla de unión entre usuarios (profesores) y sus materias';
COMMENT
ON TABLE classes IS 'Eventos. specialty_id reemplaza al specialty_tag de MVP.';
COMMENT
